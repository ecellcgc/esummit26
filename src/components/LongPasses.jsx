...
export default function LongPasses() {
  ...
  // Open Tally modal using per-pass data attributes (no screenshot modal on register)
  const openTally = (pass) => {
    try {
      const attrs = pass.buttonAttrs || {};
      const btn = document.createElement("button");
      Object.entries(attrs).forEach(([k, v]) => {
        try { btn.setAttribute(k, String(v)); } catch (e) {}
      });
      ...
      console.error("openTally failed", e);
    }
  };

  const handleRegisterClick = (pass) => {
    // Always open Tally popup for registration; do not open screenshot/payment modals here.
    openTally(pass);
  };
